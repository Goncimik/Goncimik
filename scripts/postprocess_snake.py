...
PURPLE = {
    "light": {
        "pixel": "#a855f7",   # violet-500
    },
    "dark": {
        "pixel": "#c084fc",   # violet-300
    }
}
...
def recolor_remaining_nonpurple_tiles(root: ET.Element) -> None:

    def is_greenish(hexcol: str) -> bool:
   
        s = hexcol.strip().lower()
        if not s.startswith("#"):
            return False
        if len(s) == 4:
...
def add_pixel_label(root: ET.Element, label: str, dark: bool) -> None:
    ...
    palette = PURPLE["dark" if dark else "light"]
    dot_fill = palette["pixel"]
    dot_opacity = "0.97"
...
def main():
    ...
    recolor_remaining_nonpurple_tiles(root)
